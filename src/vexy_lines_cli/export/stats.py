# this_file: vexy-lines-cli/src/vexy_lines_cli/export/stats.py
...
class ExportStats:
    ...
    Tracks success/skip/failure counts, per-file timing, and provides
    both dict and human-readable summary output.
    """
    ...
    def record_skipped(self, path: Path) -> None:
        """Record a file that was skipped (output already exists)."""
        self.processed += 1
        self.skipped += 1
    ...
    def record_validation_failure(self, path: Path, reason: str) -> None:
        """Record a validation failure on an exported file."""
        self.processed += 1
